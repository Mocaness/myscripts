...
# Devices variable
ZIPNAME="KalinaNegev-clang"
DEVICE="whyred"
DEFCONFIG=whyred_defconfig

# EnvSetup
KBUILD_BUILD_USER="Mocarafee"
KBUILD_BUILD_HOST=KotoneHouse
...
clone() {
	echo " "
	msg "|| Cloning Clang ||"
	git clone --depth=1 https://github.com/GengKapak/GengKapak-clang clang-llvm --no-tags --single-branch
...
build_kernel() {
 	...
 	make O=out $DEFCONFIG ARCH=arm64 CC="clang" CROSS_COMPILE="aarch64-linux-gnu-" CROSS_COMPILE_ARM32="arm-linux-gnueabi-"

	msg "|| Started Compilation ||"
	BUILD_START=$(date +"%s")
	make -j"$PROCS" O=out ARCH=arm64 CC="clang" CROSS_COMPILE="aarch64-linux-gnu-" CROSS_COMPILE_ARM32="arm-linux-gnueabi-"
	BUILD_END=$(date +"%s")
